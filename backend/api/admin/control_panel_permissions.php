...
if (($roleId <= 0 && $userId <= 0) || !is_array($permissionIds)) {
    http_response_code(400);
    echo json_encode(['error' => 'Either role_id or user_id and permission_ids are required']);
    exit;
}
...
try {
    ...
    if (count($cleanPermissionIds) > 0 && $roleId > 0) {
    ...
    if (count($cleanPermissionIds) > 0 && $userId > 0) {
        $insertStmt = $conn->prepare("INSERT INTO user_permissions (user_id, permission_id, is_allowed) VALUES (?, ?, 1)");
        if (!$insertStmt) {
